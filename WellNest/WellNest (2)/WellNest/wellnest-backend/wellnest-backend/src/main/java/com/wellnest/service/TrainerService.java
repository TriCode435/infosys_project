...
public class TrainerService {
        ...
        public TrainerProfileDTO getTrainerProfile(String username) {
                ...
                userProfileRepository.findByUser(user).ifPresent(p -> dto.setFullName(p.getFullName()));
                ...
                dto.setPricePerSession(profile.getPricePerSession());
        ...
        public void updateTrainerProfile(String username, TrainerProfileDTO dto) {
                ...
                UserProfile userProfile = userProfileRepository.findByUser(user)
                                .orElseGet(() -> {
                                        ...
                                        newUp.setUser(Objects.requireNonNull(user));
                                        return newUp;
                                });
        ...
        public org.springframework.data.domain.Page<TrainerProfileDTO> searchTrainers(
                        String goal, Double minRating, boolean availableOnly, Double maxPrice,
                        org.springframework.data.domain.Pageable pageable) {
                return trainerProfileRepository.searchTrainers(goal, minRating, availableOnly, maxPrice, pageable)
                                .map(this::mapToTrainerDTO);
        }

        public TrainerProfileDTO getTrainerById(Long id) {
                TrainerProfile profile = trainerProfileRepository.findById(id)
                                .orElseThrow(() -> new RuntimeException("Trainer profile not found"));
                return mapToTrainerDTO(profile);
        }

        private TrainerProfileDTO mapToTrainerDTO(TrainerProfile t) {
                TrainerProfileDTO dto = new TrainerProfileDTO();
                dto.setId(t.getUser().getId());
                dto.setUsername(t.getUser().getUsername());
                userProfileRepository.findByUser(t.getUser()).ifPresent(p -> dto.setFullName(p.getFullName()));
                ...
                dto.setBio(t.getBio());
                dto.setPricePerSession(t.getPricePerSession());
                dto.setProfileImage(t.getProfileImage());
                dto.setAvailable(t.isAvailable());
                return dto;
        }

        public void toggleAvailability(String username) {
                User user = userRepository.findByUsername(username)
                                .orElseThrow(() -> new RuntimeException("User not found"));
                TrainerProfile profile = trainerProfileRepository.findByUser(user)
        ...
        public List<TrainerProfileDTO> getSuggestedTrainers(String goal) {
                ...
                return allTrainers.stream()
                                .filter(TrainerProfile::isAvailable)
                                .filter(t -> goal == null || t.getSpecializations().isEmpty() ||
                                                t.getSpecializations().stream().anyMatch(
                                                                s -> s.toLowerCase().contains(goal.toLowerCase())))
                                .map(this::mapToTrainerDTO).collect(Collectors.toList());
        }
