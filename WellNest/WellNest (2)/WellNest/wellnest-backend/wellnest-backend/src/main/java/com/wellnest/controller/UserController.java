...
public class UserController {
    ...
    public ResponseEntity<?> getUserDashboard(
        ...
        if (role.equals("ROLE_TRAINER")) {
            if (!userService.canTrainerAccessUser(authentication.getName(), id)) {
                return ResponseEntity.status(403).body(Map.of("message", "You do not have a booking with this user"));
            }
        }

        // ADMIN has full access
        return ResponseEntity.ok(userService.getDashboardSummaryById(id, startDate, endDate));
    }
