...
// Blog moderation data fetch panna
const fetchPendingPosts = async () => {
    try {
        const res = await api.get('/api/blog/pending');
        setPendingPosts(res.data);
    } catch (e) { console.error("Blog fetch error", e); }
};

// useEffect-la ithaiyum sethu call pannu
useEffect(() => { 
    fetchData(); 
    fetchPendingPosts(); 
}, []);

// Status update (Approve/Reject) logic
const handleStatusUpdate = async (postId, status) => {
    try {
        await api.patch(`/api/blog/${postId}/status`, { status });
        alert(`Post ${status}!`);
    ...
    return (
        <div style={{ backgroundColor: '#f0f4f8', minHeight: '100vh', padding: '40px 20px', fontFamily: '"Inter", sans-serif' }}>
            <div style={{ maxWidth: '1300px', margin: '0 auto' }}>
                ...
                {/* 3.5 Blog Moderation Hub */}
<motion.section whileHover={{ y: -5 }} style={{ ...glassStyle, marginTop: '30px', gridColumn: '1 / -1' }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
        <div style={{ background: '#f59e0b', padding: '10px', borderRadius: '12px', color: 'white' }}><MessageSquare size={20} /></div>
        <h3 style={{ fontSize: '20px', fontWeight: '900', color: '#1e293b', margin: 0 }}>CONTENT MODERATION <span style={{color: '#94a3b8', fontSize: '14px'}}>({pendingPosts.length} Pending)</span></h3>
    </div>

    {pendingPosts.length === 0 ? (
        <p style={{ color: '#94a3b8', fontSize: '13px', fontWeight: '600' }}>No pending posts for review.</p>
    ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
            {pendingPosts.map(post => (
                <div key={post.id} style={{ background: '#f8fafc', padding: '20px', borderRadius: '20px', border: '1px solid #e2e8f0' }}>
                    <h4 style={{ margin: '0 0 10px 0', color: '#1e293b' }}>{post.title}</h4>
                    <p style={{ fontSize: '13px', color: '#64748b', marginBottom: '15px' }}>{post.content.substring(0, 100)}...</p>
                    <div style={{ display: 'flex', gap: '10px' }}>
                        <button 
                            onClick={() => handleStatusUpdate(post.id, 'APPROVED')}
                            style={{ flex: 1, background: '#10b981', color: 'white', border: 'none', padding: '10px', borderRadius: '12px', fontWeight: 'bold', cursor: 'pointer' }}
                        >
                            APPROVE
                        </button>
                        <button 
                            onClick={() => handleStatusUpdate(post.id, 'REJECTED')}
                            style={{ flex: 1, background: '#ef4444', color: 'white', border: 'none', padding: '10px', borderRadius: '12px', fontWeight: 'bold', cursor: 'pointer' }}
                        >
                            REJECT
                        </button>
                    </div>
